...
%
%	For the gaussian window, an optional parameter K
...
elseif strcmp(name,'BLACKMANHARRIS'),
 ind=(-(N-1)/2:(N-1)/2)' *2.0*pi/N;
...
elseif strcmp(name,'POISSON'),
 if (nargin==3), K=param; else K=4; end;
ind=abs(-(N-1)/2:(N-1)/2)'*2/N;
h=exp(-K.*ind);
% Hanning-Poisson window
elseif strcmp(name,'HANNINGPOISSON'),
    if (nargin==3), K=param; else K=2; end;
ind=(-(N-1)/2:(N-1)/2)'*2/N;
h=0.50.*(1+ cos(pi.*ind)).*exp(-K.*abs(ind));
% Cauchy window
elseif strcmp(name,'CAUCHY'),
 if (nargin==3), K=param; else K=4; end;
ind=(-(N-1)/2:(N-1)/2)'*2/N;
h=1./(1+abs(K*ind).^2);
